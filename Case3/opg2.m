...
mean_M=mean(data_ulod_M(M:end-M)) % målt avg

var_calc_M=var_meas(1)/M % forventet varians
rms_calc_M=sqrt(var_calc_M) % forventet rms

rms_M=std(data_ulod_M(M:end-M)) % målt rms
var_M=rms_M^2 % målt varians
% plot af målt og uregnet rms. og filtret data
figure
